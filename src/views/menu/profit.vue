  ...
  <div class="output"
    ...
    <el-card class="box-card">
      <el-button size="mini"
                 type="primary"
                 @click="add"
                 v-if="pandun(302)">新增</el-button>
      <el-button size="mini"
                 type="danger"
                 plain
                 @click="del"
                 :disabled="!selId.length"
                 v-if="pandun(301)">删除</el-button>
      <el-button size="mini"
                 plain
                 v-if="pandun(310)"
        ...
        <el-table :data="tableData"
          ...
          <el-table-column align="center"
            ...
            <template slot-scope="scope">
              <el-button size="mini"
                         @click="edit(scope)"
                         v-if="setStatus(scope.row,'edit') && pandun(303)">编辑</el-button>
              <el-button size="mini"
                         :loading="btnLoading == '驳回'"
                         :disabled="!!btnLoading"
                         @click="changeStatus(scope.row,'驳回',307)"
                         v-if="setStatus(scope.row,'boh') && pandun(307)">驳回</el-button>
              <el-button size="mini"
                         :loading="btnLoading == '审核中'"
                         :disabled="!!btnLoading"
                         @click="changeStatus(scope.row,'审核中',305)"
                         v-if="setStatus(scope.row,'tj') && pandun(305)">提交</el-button>
              <el-button size="mini"
                         :loading="btnLoading == '已审核'"
                         :disabled="!!btnLoading"
                         @click="changeStatus(scope.row,'已审核',309)"
                         v-if="setStatus(scope.row,'tg') && pandun(309)">通过</el-button>
              <el-button size="mini"
                         :loading="btnLoading == '草稿'"
                         :disabled="!!btnLoading"
                         @click="changeStatus(scope.row,'草稿',308)"
                         v-if="setStatus(scope.row,'cx') && pandun(308)">撤销</el-button>
              <el-button size="mini"
                         :loading="btnLoading == '停用'"
                         :disabled="!!btnLoading"
                         @click="changeStatus(scope.row,'停用',306)"
                         v-if="setStatus(scope.row,'ty') && pandun(306)">停用</el-button>
            </template>
          </el-table-column>
    ...
    <infoDio v-model="tableShow"
             isDefaultKind="profit"
             :eidArr="formDesc.eid.options"
             @success="successDio"
             drawerTitle="利润信息设置"></infoDio>
  </div>
</template>
...
import { proTable } from "@/common/table";
...
export default {
  ...
  methods: {
    ...
    async changeStatus(v, s, flag) {
      ...
      try {
        const result = await this.$api.proUpdateStatus(this, parm);
    ...
    del() {
      this.$confirm("此操作将删除该信息, 是否继续?", "提示", {
        ...
        try {
          await this.$api.proDel(this, { ids: this.selId });
          this.getAllTable();
    ...
    async getAllTable(v = false) {
      this.initTable = true;
      let name = this.pandun(304) ? "proGetAllTable" : "proFindOnlyID";
    ...
    async saveAdd() {
      try {
        const result = await this.$api.proAdd(this, {
          ...this.formData,
          status: "草稿",
    ...
    async saveEdit() {
      try {
        ...
        const result = await this.$api.proUpdate(this, param);
        this.drawer = false;
        this.getAllTable(true);
        this.$message.success(result.desc);
      } catch (error) {
