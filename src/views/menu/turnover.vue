  ...
  <div class="output"
    ...
    <el-card class="box-card">
      <el-button size="mini"
                 type="primary"
                 @click="add"
                 v-if="pandun(202)">新增</el-button>
      <el-button size="mini"
                 type="danger"
                 plain
                 @click="del"
                 :disabled="!selId.length"
                 v-if="pandun(201)">删除</el-button>
      <el-button size="mini"
                 plain
                 v-if="pandun(210)"
        ...
        <el-table :data="tableData"
          ...
          <el-table-column align="center"
            ...
            <template slot-scope="scope">
              <el-button size="mini"
                         @click="edit(scope)"
                         v-if="setStatus(scope.row,'edit') && pandun(203)">编辑</el-button>
              <el-button size="mini"
                         :loading="btnLoading == '驳回'"
                         :disabled="!!btnLoading"
                         @click="changeStatus(scope.row,'驳回',207)"
                         v-if="setStatus(scope.row,'boh') && pandun(207)">驳回</el-button>
              <el-button size="mini"
                         :loading="btnLoading == '审核中'"
                         :disabled="!!btnLoading"
                         @click="changeStatus(scope.row,'审核中',205)"
                         v-if="setStatus(scope.row,'tj') && pandun(205)">提交</el-button>
              <el-button size="mini"
                         :loading="btnLoading == '已审核'"
                         :disabled="!!btnLoading"
                         @click="changeStatus(scope.row,'已审核',209)"
                         v-if="setStatus(scope.row,'tg') && pandun(209)">通过</el-button>
              <el-button size="mini"
                         :loading="btnLoading == '草稿'"
                         :disabled="!!btnLoading"
                         @click="changeStatus(scope.row,'草稿',208)"
                         v-if="setStatus(scope.row,'cx') && pandun(208)">撤销</el-button>
              <el-button size="mini"
                         :loading="btnLoading == '停用'"
                         :disabled="!!btnLoading"
                         @click="changeStatus(scope.row,'停用',206)"
                         v-if="setStatus(scope.row,'ty') && pandun(206)">停用</el-button>
            </template>
          </el-table-column>
    ...
    <infoDio v-model="tableShow"
             isDefaultKind="turnover"
             :eidArr="formDesc.eid.options"
             @success="successDio"
             drawerTitle="营业额信息设置"></infoDio>
  </div>
</template>
...
import { turnTable } from "@/common/table";
...
export default {
  ...
  methods: {
    ...
    async changeStatus(v, s, flag) {
      ...
      try {
        const result = await this.$api.turnUpdateStatus(this, parm);
    ...
    del() {
      this.$confirm("此操作将删除该信息, 是否继续?", "提示", {
        ...
        try {
          await this.$api.turnDel(this, { ids: this.selId });
          this.getAllTable();
    ...
    async getAllTable(v = false) {
      this.initTable = true;
      let name = this.pandun(204) ? "turnGetAllTable" : "turnFindOnlyID";
    ...
    async saveAdd() {
      try {
        const result = await this.$api.turnAdd(this, {
          ...this.formData,
          status: "草稿",
    ...
    async saveEdit() {
      try {
        ...
        const result = await this.$api.turnUpdate(this, param);
        this.drawer = false;
        this.getAllTable(true);
        this.$message.success(result.desc);
      } catch (error) {
